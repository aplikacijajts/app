# JTS Logistics INC Dispatch/TMS — Command Center v2.6.0

Premium original Dispatch/TMS web application branded for JTS Logistics INC.

This package preserves the existing native Node.js / vanilla-JS TMS architecture and is prepared for controlled production testing. The bundled database contains no demo operational records.
...
## Command Center v2.6.0

This release is an in-place production hardening and visual command-center pass. Existing Dispatch Board, Load Workspace, Driver Pay, Documents/OCR, reminders, chat, notifications, WebRTC voice calls, reporting, financial calculations and PWA behavior remain on the existing architecture.

### Mobile session and PWA
- Secure session restore now has a 30-second fetch timeout and an unconditional loader cleanup path, so a stalled `/api/data` request cannot leave the **Restoring secure session** screen permanently visible.
- Production mobile browsers are blocked until the app is installed. There is no production Skip action. Android uses the real `beforeinstallprompt` event when available; iOS uses Safari Share → Add to Home Screen instructions.

### Location and privacy
- Drivers see an explicit **Enable Location Sharing** control before GPS starts.
- Location sharing uses an authenticated server-side session, derives Driver identity from the session, throttles updates, validates accuracy and rejects obvious GPS outliers.
- Current locations are server-scoped to assigned Dispatchers and active Load relationships for Brokers.
- Browser GPS is never treated as an ELD record. Possible Rest requires at least 90 minutes of qualified stationary samples and Driver confirmation.
- Location retention is configurable through `LOCATION_RETENTION_HOURS` and defaults to 72 hours.

### Maps, ETA and operational alerts
- Admin, Dispatcher and Broker GPS pages use Leaflet with OpenStreetMap tiles, marker clustering and an accessible list fallback.
- ETA is an operational estimate based on available load mileage, location freshness and configured average speed; it does not claim live traffic or authoritative routing.
- Delay and freshness states are deduplicated before notification.

### Broker operational messaging
Brokers can send an operational message only through an authorized active Load relationship. The server resolves the assigned Dispatcher and routes the message to active Admin users and that Dispatcher; frontend-supplied Dispatcher IDs are ignored.

### Environment configuration
Additional optional production controls:

```text
LOCATION_RETENTION_HOURS=72
LOCATION_UPDATE_MIN_MS=15000
LOCATION_MIN_DISTANCE_METERS=35
REST_MIN_DURATION_MINUTES=90
REST_STATIONARY_RADIUS_METERS=120
REST_MAX_ACCURACY_METERS=80
REST_MAX_SAMPLE_AGE_MINUTES=15
REST_MIN_VALID_SAMPLES=12
REST_MAX_GAP_MINUTES=12
```

Run the migration after upgrading an existing persistent database:

```bash
node scripts/migrate-v2.6.js
```

The Service Worker cache identifier is `jts-tms-v26-command-center`; old caches are deleted during activation.


### v2.6.1 navigation refinement
- Driver role now has a dedicated **Driver Location** menu item in the same position formerly occupied by Chat.
- The Driver mobile bottom navigation uses Driver Location in that slot.
- Chat functionality remains preserved in the application and is not removed from the underlying messaging infrastructure.
