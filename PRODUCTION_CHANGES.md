...
## v2.6.0 final hardening

- Added secure location-sharing sessions with server-derived Driver identity.
- Scoped Dispatcher location data to assigned Drivers and Broker location data to active-load relationships.
- Added configurable retention and GPS validation/throttling.
- Added qualified 90-minute Possible Rest detection with Driver confirmation. GPS inactivity remains advisory and never overwrites authoritative ELD data.
- Replaced the old iframe-only GPS page with Leaflet/OpenStreetMap command-center maps with marker clustering and list fallback.
- Added operational ETA/delay state using transparent average-speed estimation without fabricated traffic data.
- Added Broker operational messaging routed server-side to active Admins and the assigned Dispatcher.
- Added JTS branded session restoration loader timeout/finally cleanup to prevent permanent mobile loading screens.
