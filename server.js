...
const LOCATION_RETENTION_HOURS = Math.max(1, Number(process.env.LOCATION_RETENTION_HOURS || 72));
const LOCATION_UPDATE_MIN_MS = Math.max(5000, Number(process.env.LOCATION_UPDATE_MIN_MS || 15000));
const LOCATION_MIN_DISTANCE_METERS = Math.max(0, Number(process.env.LOCATION_MIN_DISTANCE_METERS || 35));
const REST_MIN_DURATION_MINUTES = Math.max(90, Number(process.env.REST_MIN_DURATION_MINUTES || 90));
const REST_STATIONARY_RADIUS_METERS = Math.max(20, Number(process.env.REST_STATIONARY_RADIUS_METERS || 120));
const REST_MAX_ACCURACY_METERS = Math.max(25, Number(process.env.REST_MAX_ACCURACY_METERS || 80));
const REST_MAX_SAMPLE_AGE_MINUTES = Math.max(30, Number(process.env.REST_MAX_SAMPLE_AGE_MINUTES || 15));
const REST_MIN_VALID_SAMPLES = Math.max(6, Number(process.env.REST_MIN_VALID_SAMPLES || 12));
const REST_MAX_GAP_MINUTES = Math.max(5, Number(process.env.REST_MAX_GAP_MINUTES || 12));
...
const allowedCollections = new Set(['loads', 'drivers', 'fleet', 'brokers', 'docs', 'notifications', 'activities', 'locations', 'hosLogs', 'intake', 'auditLog', 'reminders', 'docFolders', 'driverPayAdjustments', 'restEvents', 'delayAlerts']);
...
function normalizeDb(db = {}) {
  ...
  out.locations = out.locations.map(item => ({ ...item, id: sanitizeText(item.id, makeId('loc')), createdAt: item.createdAt || item.timestamp || new Date().toISOString(), timestamp: item.timestamp || item.createdAt || new Date().toISOString(), serverTimestamp: item.serverTimestamp || item.createdAt || item.timestamp || new Date().toISOString(), driverUserId: sanitizeText(item.driverUserId || ''), driverEmail: sanitizeText(item.driverEmail || '').toLowerCase(), loadId: sanitizeText(item.loadId || ''), sharingActive: item.sharingActive !== false, accuracy: Math.max(0, Number(item.accuracy || 0)), speed: Number(item.speed || 0) })).slice(0, 4000);
  out.locationSessions = (Array.isArray(out.locationSessions) ? out.locationSessions : []).map(item => ({ ...item, id: sanitizeText(item.id, makeId('ls')), active: Boolean(item.active), driverUserId: sanitizeText(item.driverUserId || ''), driverEmail: sanitizeText(item.driverEmail || '').toLowerCase(), createdAt: item.createdAt || new Date().toISOString(), updatedAt: item.updatedAt || item.createdAt || new Date().toISOString() })).slice(0, 500);
  out.restEvents = (Array.isArray(out.restEvents) ? out.restEvents : []).map(item => ({ ...item, id: sanitizeText(item.id, makeId('rest')), driverUserId: sanitizeText(item.driverUserId || ''), driverEmail: sanitizeText(item.driverEmail || '').toLowerCase(), state: sanitizeText(item.state || 'Awaiting Confirmation'), startAt: sanitizeText(item.startAt || item.detectedAt || new Date().toISOString()) })).slice(0, 1000);
  out.delayAlerts = Array.isArray(out.delayAlerts) ? out.delayAlerts.slice(0, 1000) : [];
...
function activeLocationSession(db, driverUser) {
  const email = sanitizeText(driverUser?.email).toLowerCase();
  const driverId = sanitizeText(driverUser?.id);
  return (db.locationSessions || []).find(item => item.active && ((driverId && item.driverUserId === driverId) || (email && item.driverEmail === email))) || null;
}
function locationAccessForUser(db, user, loc = {}) {
  if (!user || !loc) return false;
  if (user.role === 'admin') return true;
  if (user.role === 'driver') return sanitizeText(loc.driverUserId) === sanitizeText(user.id) || sanitizeText(loc.driverEmail).toLowerCase() === sanitizeText(user.email).toLowerCase();
  if (user.role === 'dispatcher') {
    const driver = (db.users || []).find(item => item.role === 'driver' && (sanitizeText(item.id) === sanitizeText(loc.driverUserId) || sanitizeText(item.email).toLowerCase() === sanitizeText(loc.driverEmail).toLowerCase()));
    const assigned = assignedDispatcherForDriver(db, driver);
    return Boolean(assigned && assigned.id === user.id);
  }
  if (user.role === 'broker') {
    const load = db.loads.find(item => String(item.id) === String(loc.loadId || ''));
    return Boolean(load && loadVisibleToUser(db, user, load) && !String(loc.offDuty || '').toLowerCase().includes('true'));
  }
  return false;
}
function cleanupLocationHistory(db) {
  const cutoff = Date.now() - LOCATION_RETENTION_HOURS * 60 * 60 * 1000;
  db.locations = (Array.isArray(db.locations) ? db.locations : []).filter(item => new Date(item.serverTimestamp || item.timestamp || item.createdAt || 0).getTime() >= cutoff).slice(0, 4000);
}
function resolveDriverUserReference(db, ref) { const clean=sanitizeText(ref).toLowerCase(); if(!clean)return null; return (db.users||[]).find(u=>u.role==='driver'&&([u.id,u.email,u.name].map(v=>sanitizeText(v).toLowerCase()).includes(clean))) || (db.drivers||[]).reduce((found,d)=> found || (sanitizeText(d.id).toLowerCase()===clean ? (db.users||[]).find(u=>u.role==='driver'&&(sanitizeText(u.email).toLowerCase()===sanitizeText(d.email).toLowerCase()||sanitizeText(u.name).toLowerCase()===sanitizeText(d.name).toLowerCase())) : null),null); }
function resolveDriverForSession(db, authUser) {
  const user = db.users.find(item => item.id === authUser.id && item.role === 'driver' && isActiveUser(item));
  const driver = findDriverForUser(db, authUser);
  return user && driver ? { user, driver } : null;
}
function relatedLoadForDriver(db, driver) {
  return db.loads.find(load => !DELIVERED_STATUSES.has(String(load.status || '').trim()) && (sanitizeText(load.driverEmail).toLowerCase() === sanitizeText(driver.email).toLowerCase() || sanitizeText(load.driver).toLowerCase() === sanitizeText(driver.name).toLowerCase())) || null;
}
function restSequenceForDriver(db, driverUser) {
  const cutoff = Date.now() - (REST_MIN_DURATION_MINUTES + 90) * 60 * 1000;
  return (db.locations || []).filter(loc => (sanitizeText(loc.driverUserId) === sanitizeText(driverUser.id) || sanitizeText(loc.driverEmail).toLowerCase() === sanitizeText(driverUser.email).toLowerCase()) && loc.sharingActive !== false && new Date(loc.timestamp || loc.createdAt || 0).getTime() >= cutoff).sort((a,b) => new Date(a.timestamp||a.createdAt||0)-new Date(b.timestamp||b.createdAt||0));
}
function detectPossibleRest(db, driverUser) {
  const seq = restSequenceForDriver(db, driverUser);
  if (!seq.length) return { status: 'Insufficient Data', event: null };
  const now = Date.now();
  const valid = seq.filter(sample => {
    return Number.isFinite(Number(sample.lat)) && Number.isFinite(Number(sample.lng)) && Number(sample.accuracy || 999) <= REST_MAX_ACCURACY_METERS;
  });
  if (valid.length < REST_MIN_VALID_SAMPLES) return { status: 'Insufficient Data', event: null };
  const first = valid[0];
  const last = valid[valid.length - 1];
  const lastSampleAge = now - new Date(last.timestamp || last.createdAt || 0).getTime();
  if (!Number.isFinite(lastSampleAge) || lastSampleAge > REST_MAX_SAMPLE_AGE_MINUTES * 60 * 1000) return { status: 'Location Stale', event: null };
  const durationMs = new Date(last.timestamp || last.createdAt).getTime() - new Date(first.timestamp || first.createdAt).getTime();
  if (durationMs < REST_MIN_DURATION_MINUTES * 60 * 1000) return { status: 'Stationary', event: null };
  let maxDistance = 0; let longestGap = 0; let reliableSpeed = 0;
  for (let i = 1; i < valid.length; i += 1) {
    maxDistance = Math.max(maxDistance, haversineMeters(first, valid[i]));
    const prevT = new Date(valid[i-1].timestamp || valid[i-1].createdAt).getTime();
    const curT = new Date(valid[i].timestamp || valid[i].createdAt).getTime();
    longestGap = Math.max(longestGap, curT - prevT);
    const speed = Number(valid[i].speed);
    if (Number.isFinite(speed)) reliableSpeed = Math.max(reliableSpeed, speed);
    if (haversineMeters(valid[i-1], valid[i]) > REST_STATIONARY_RADIUS_METERS * 3) return { status: 'Moving', event: null };
  }
  if (maxDistance > REST_STATIONARY_RADIUS_METERS || longestGap > REST_MAX_GAP_MINUTES * 60 * 1000 || reliableSpeed > 2.5) return { status: 'Moving', event: null };
  const existing = (db.restEvents || []).find(event => event.driverUserId === driverUser.id && ['Possible Rest','Awaiting Confirmation'].includes(event.state) && Math.abs(new Date(event.startAt).getTime() - new Date(first.timestamp || first.createdAt).getTime()) < 5 * 60 * 1000);
  const load = relatedLoadForDriver(db, findDriverForUser(db, driverUser) || {});
  if (existing) return { status: 'Awaiting Confirmation', event: existing };
  const event = {
    id: makeId('rest'), driverUserId: driverUser.id, driverEmail: driverUser.email, driverName: driverUser.name,
    loadId: load?.id || '', dispatcherId: load ? assignedDispatcherForDriver(db, driverUser)?.id || '' : '',
    startAt: first.timestamp || first.createdAt, detectedAt: new Date().toISOString(), endAt: last.timestamp || last.createdAt,
    durationMinutes: Math.round(durationMs / 60000), radiusMeters: Math.round(maxDistance), sampleCount: valid.length,
    maxAccuracy: Math.max(...valid.map(v => Number(v.accuracy || 0))), longestGapMinutes: Math.round(longestGap / 60000),
    source: 'browser-gps-stationary-detection', state: 'Awaiting Confirmation', classification: '', note: '', confirmedAt: '', confirmedBy: ''
  };
  db.restEvents.unshift(event); db.restEvents = db.restEvents.slice(0, 1000);
  addNotification(db, { title: 'Possible Rest · Confirmation Required', text: `${driverUser.name} remained stationary for about ${event.durationMinutes} minutes. Driver confirmation is required.`, type: 'Possible Rest', target: driverUser.email, targetName: driverUser.name, role: 'driver', relatedLoadId: event.loadId, relatedPage: 'driver-mobile', action: 'review-rest', createdBy: 'system' });
  const dispatcher = assignedDispatcherForDriver(db, driverUser);
  if (dispatcher) addNotification(db, { title: 'Possible Rest · Review Required', text: `${driverUser.name} has a possible rest period awaiting confirmation.`, type: 'Possible Rest', target: dispatcher.email, targetName: dispatcher.name, role: 'dispatcher', relatedLoadId: event.loadId, relatedPage: 'eld', action: 'review-rest', createdBy: 'system' });
  logAudit(db, 'Possible rest detected', `${driverUser.name} ${event.startAt} ${event.durationMinutes}m`, 'system');
  return { status: 'Awaiting Confirmation', event };
}
function operationalEstimateForLoad(db, load) {
  if (!load) return { status: 'Insufficient Data' };
  const driver = db.drivers.find(d => sanitizeText(d.name).toLowerCase() === sanitizeText(load.driver).toLowerCase() || sanitizeText(d.email).toLowerCase() === sanitizeText(load.driverEmail).toLowerCase());
  const loc = (db.locations || []).filter(x => String(x.loadId || '') === String(load.id) && Number.isFinite(Number(x.lat)) && Number.isFinite(Number(x.lng))).sort((a,b) => new Date(b.timestamp||b.createdAt)-new Date(a.timestamp||a.createdAt))[0];
  const now = Date.now(); const freshnessMs = loc ? now - new Date(loc.timestamp || loc.createdAt).getTime() : Infinity;
  if (!loc || freshnessMs > 45 * 60 * 1000) return { status: 'Location Stale', loadId: load.id };
  const miles = Number(load.miles || load.driverMiles || 0); const mph = Math.max(20, Number(driver?.averageMph || db.company.defaultAverageMph || 55));
  const travelHours = miles > 0 ? miles / mph : 0;
  const appointment = load.deliveryDate || load.deliveryTime ? new Date(`${load.deliveryDate || ''}T${load.deliveryTime || '23:59'}`).getTime() : NaN;
  const etaAt = new Date(now + travelHours * 3600000).toISOString();
  const delayMinutes = Number.isFinite(appointment) ? Math.round((now + travelHours * 3600000 - appointment) / 60000) : 0;
  return { loadId: load.id, eta: etaAt, delayMinutes: Math.max(0, delayMinutes), status: delayMinutes > 30 ? 'Delayed' : delayMinutes > 0 ? 'At Risk' : 'On Time', method: 'operational-average-speed', freshMinutes: Math.round(freshnessMs/60000), hOSSource: db.company.eldProvider === 'manual' ? 'Estimated HOS' : 'ELD' };
}
...
function visibleCollection(db, user, collection) {
  ...
  if (user?.role === 'dispatcher') {
    ...
    if (collection === 'loads') return db.loads.filter(load => { const driver = db.users.find(u => u.role === 'driver' && (sanitizeText(u.email).toLowerCase() === sanitizeText(load.driverEmail).toLowerCase() || sanitizeText(u.name).toLowerCase() === sanitizeText(load.driver).toLowerCase())); return !load.driver && !load.driverEmail ? false : Boolean(driver && assignedDispatcherForDriver(db, driver)?.id === user.id); }).map(load => sanitizeLoadForUser(load, user));
    if (collection === 'drivers') return assignedDriversForDispatcher(db, user).map(driver => db.drivers.find(d => sanitizeText(d.email).toLowerCase() === sanitizeText(driver.email).toLowerCase() || sanitizeText(d.name).toLowerCase() === sanitizeText(driver.name).toLowerCase())).filter(Boolean);
    if (collection === 'fleet') { const assignedDriverNames = new Set(assignedDriversForDispatcher(db,user).flatMap(u=>[sanitizeText(u.name).toLowerCase(),sanitizeText(u.email).toLowerCase()])); return db.fleet.filter(unit=>assignedDriverNames.has(sanitizeText(unit.driver).toLowerCase())); }
    if (collection === 'locations') return db.locations.filter(item => locationAccessForUser(db, user, item)).slice(0,200);
    if (collection === 'restEvents') return db.restEvents.filter(item => item.dispatcherId === user.id).slice(0,100);
    if (collection === 'delayAlerts') return db.delayAlerts.filter(item => { const load=db.loads.find(l=>String(l.id)===String(item.loadId)); return load && load.driverEmail && db.users.some(u=>u.role==='driver' && sanitizeText(u.email).toLowerCase()===sanitizeText(load.driverEmail).toLowerCase() && assignedDispatcherForDriver(db,u)?.id===user.id); }).slice(0,100);
    return db[collection];
  }
  ...
  if (user.role === 'driver') {
    ...
    if (collection === 'notifications') return db.notifications.filter(item => notificationVisibleToUser(item, user)).map(item => decorateNotificationForUser(item, user));
    if (collection === 'locations') return db.locations.filter(item => locationAccessForUser(db, user, item)).slice(0, 200);
    if (collection === 'restEvents') return db.restEvents.filter(item => item.driverUserId === user.id || item.driverEmail === user.email).slice(0, 100);
    if (collection === 'delayAlerts') return [];
  ...
  if (user.role === 'broker') {
    ...
    if (collection === 'notifications') return db.notifications.filter(item => notificationVisibleToUser(item, user)).map(item => decorateNotificationForUser(item, user));
    if (collection === 'activities') return db.activities.filter(item => loadIds.has(String(item.loadId || item.load))).slice(0, 250);
    if (collection === 'locations') return db.locations.filter(item => locationAccessForUser(db, user, item)).slice(0,100);
    if (collection === 'drivers') return db.drivers.filter(driver => ownLoads.some(load => String(load.id)===String(driver.loadId||'') || sanitizeText(load.driver).toLowerCase()===sanitizeText(driver.name).toLowerCase())).map(({id,name,truck,trailer,status})=>({id,name,truck,trailer,status}));
    if (collection === 'fleet' || collection === 'restEvents' || collection === 'delayAlerts' || collection === 'hosLogs' || collection === 'auditLog' || collection === 'intake' || collection === 'reminders' || collection === 'docFolders' || collection === 'driverPayAdjustments') return [];
  }
...
function filteredDb(db, user) {
  ...
  output.restEvents = visibleCollection(db, user, 'restEvents');
  output.delayAlerts = visibleCollection(db, user, 'delayAlerts');
...
async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && pathName === '/api/broker-message') {
      if (authUser.role !== 'broker') return sendJson(res, 403, { error: 'Only Brokers can use the operational broker message flow.' });
      const body = await readJsonBody(req); const driverUser = resolveDriverUserReference(db, body.driverId || body.driverEmail || body.driverName); const text = sanitizeText(body.text || '').slice(0,2000);
      if (!driverUser || !text) return sendJson(res,400,{error:'An authorized Driver and message are required.'});
      const driverRecord = findDriverForUser(db, driverUser); const load = db.loads.find(item=>!DELIVERED_STATUSES.has(String(item.status || '').trim()) && loadVisibleToUser(db, authUser, item) && (sanitizeText(item.driverEmail).toLowerCase()===driverUser.email || sanitizeText(item.driver).toLowerCase()===sanitizeText(driverRecord?.name).toLowerCase()));
      if (!load) return sendJson(res,403,{error:'That Driver is not authorized through an active broker Load relationship.'});
      const dispatcher = assignedDispatcherForDriver(db, driverUser); const admins = db.users.filter(user=>user.role==='admin' && isActiveUser(user)); const recipients = [...admins, ...(dispatcher ? [dispatcher] : [])];
      const threadKey = brokerChatThreadKey(authUser); if (!Array.isArray(db.chats[threadKey])) db.chats[threadKey]=[];
      const message={id:makeId('msg'),type:'operational-broker',text,time:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),createdAt:new Date().toISOString(),user:authUser.name,userEmail:authUser.email,to:threadKey,loadId:load.id,brokerId:authUser.id,driverUserId:driverUser.id,readBy:userReadKeys(authUser)}; db.chats[threadKey].push(message); db.chats[threadKey]=db.chats[threadKey].slice(-300);
      recipients.forEach(user=>addNotification(db,{title:'Broker operational message',text:`${authUser.name}: ${text}`,type:'Broker message',target:user.email,targetName:user.name,role:user.role,relatedLoadId:load.id,relatedChatContact:threadKey,relatedPage:'chat',createdBy:authUser.email,dedupeWindowMs:0}));
      addActivity(db,{loadId:load.id,title:'Broker operational message',text:`Message from ${authUser.name} routed to operations.`,actor:authUser.email,type:'broker-message',createdBy:authUser.email}); logAudit(db,'Broker operational message',`${authUser.email} -> ${driverUser.email} / ${load.id}`,authUser.email); writeDb(db); sendJson(res,201,{ok:true,message,recipients:recipients.map(u=>({name:u.name,role:u.role})),loadId:load.id}); return;
    }
    ...
    if (req.method === 'POST' && pathName === '/api/location/session') {
      if (authUser.role !== 'driver') return sendJson(res, 403, { error: 'Only authenticated Drivers can create a location-sharing session.' });
      const resolved = resolveDriverForSession(db, authUser);
      if (!resolved) return sendJson(res, 404, { error: 'Driver profile not found.' });
      const existing = activeLocationSession(db, authUser);
      if (existing) return sendJson(res, 200, { ok: true, session: existing });
      const now = new Date().toISOString();
      const session = { id: makeId('ls'), driverUserId: authUser.id, driverEmail: authUser.email, driverName: authUser.name, active: true, startedAt: now, updatedAt: now, revokedAt: '', source: 'browser-gps' };
      db.locationSessions.unshift(session); db.locationSessions = db.locationSessions.slice(0, 500);
      logAudit(db, 'Location sharing started', authUser.email, authUser.email); writeDb(db);
      sendJson(res, 201, { ok: true, session }); return;
    }
    if (req.method === 'POST' && pathName === '/api/location/stop') {
      const session = activeLocationSession(db, authUser);
      if (!session) return sendJson(res, 200, { ok: true, stopped: false });
      session.active = false; session.revokedAt = new Date().toISOString(); session.updatedAt = session.revokedAt;
      const cutoff = Date.now() - LOCATION_RETENTION_HOURS * 60 * 60 * 1000;
      db.locations = db.locations.filter(loc => !(loc.driverUserId === authUser.id && new Date(loc.timestamp || loc.createdAt).getTime() >= cutoff));
      logAudit(db, 'Location sharing stopped', authUser.email, authUser.email); writeDb(db);
      sendJson(res, 200, { ok: true, stopped: true }); return;
    }
    if (req.method === 'POST' && pathName === '/api/location/update') {
      const body = await readJsonBody(req);
      if (authUser.role !== 'driver') return sendJson(res, 403, { error: 'Only Drivers may submit browser GPS updates.' });
      const session = activeLocationSession(db, authUser);
      const driverRecord = findDriverForUser(db, authUser);
      if (!session || !driverRecord) return sendJson(res, 409, { error: 'Start an active location-sharing session before sending GPS updates.' });
      if (/off\s*duty|sleeper\s*berth/i.test(sanitizeText(driverRecord.hosStatus || ''))) return sendJson(res, 409, { error: 'Location sharing is paused while the Driver is marked Off Duty/Sleeper Berth.' });
      const lat = Number(body.lat || body.latitude); const lng = Number(body.lng || body.longitude); const accuracy = Number(body.accuracy || 0);
      if (!Number.isFinite(lat) || !Number.isFinite(lng) || Math.abs(lat) > 90 || Math.abs(lng) > 180) return sendJson(res, 400, { error: 'Valid latitude and longitude are required.' });
      if (accuracy && accuracy > 250) return sendJson(res, 422, { error: 'GPS accuracy is too low for an operational update.' });
      const last = db.locations.find(item => item.driverUserId === authUser.id);
      const nowMs = Date.now();
      if (last) {
        const lastMs = new Date(last.serverTimestamp || last.timestamp || 0).getTime();
        if (lastMs && nowMs - lastMs < LOCATION_UPDATE_MIN_MS && haversineMeters(last, {lat,lng}) < LOCATION_MIN_DISTANCE_METERS) return sendJson(res, 200, { ok: true, throttled: true, location: last });
        if (lastMs && nowMs - lastMs < LOCATION_UPDATE_MIN_MS) return sendJson(res, 200, { ok: true, throttled: true, location: last });
        if (haversineMeters(last, {lat,lng}) > 10000 && Number(body.speed || 0) < 45) return sendJson(res, 422, { error: 'GPS outlier rejected.' });
      }
      const activeLoad = relatedLoadForDriver(db, driverRecord); const timestamp = new Date().toISOString();
      const loc = prepareRecord('locations', { id: makeId('loc'), driver: driverRecord.name, driverUserId: authUser.id, driverEmail: authUser.email, loadId: activeLoad?.id || '', dispatcherId: assignedDispatcherForDriver(db, authUser)?.id || '', truckId: sanitizeText(driverRecord.truck || activeLoad?.truck || ''), trailerId: sanitizeText(driverRecord.trailer || activeLoad?.trailer || ''), lat, lng, speed: Number(body.speed || 0), heading: Number(body.heading || 0), accuracy, source: 'browser-gps', timestamp, serverTimestamp: timestamp, sharingActive: true, createdBy: authUser.email });
      db.locations.unshift(loc); cleanupLocationHistory(db);
      driverRecord.lastLat = lat; driverRecord.lastLng = lng; driverRecord.lastLocationAt = timestamp; driverRecord.lastLocationSource = 'browser-gps';
      session.updatedAt = timestamp;
      const rest = detectPossibleRest(db, authUser);
      if (activeLoad) {
        const estimate = operationalEstimateForLoad(db, activeLoad); activeLoad.operationalEta = estimate.eta || ''; activeLoad.operationalDelayMinutes = estimate.delayMinutes || 0; activeLoad.operationalRisk = estimate.status || ''; activeLoad.operationalEtaSource = estimate.method || '';
        if (['Delayed','At Risk'].includes(estimate.status)) {
          const key = `${activeLoad.id}:${estimate.status}:${Math.floor((estimate.delayMinutes || 0) / 30)}`;
          const recent = (db.delayAlerts || []).find(a => a.dedupeKey === key && Date.now() - new Date(a.createdAt).getTime() < 60 * 60 * 1000);
          if (!recent) { const alert={id:makeId('delay'),dedupeKey:key,loadId:activeLoad.id,driverUserId:authUser.id,status:'Open',severity:estimate.status==='Delayed'?'high':'medium',title:`${estimate.status} · ${activeLoad.id}`,text:`Operational ETA currently indicates ${Math.max(0,estimate.delayMinutes||0)} minute(s) of delay.`,createdAt:timestamp}; db.delayAlerts.unshift(alert); addNotification(db,{title:alert.title,text:alert.text,type:'Delay Alert',targetRole:'dispatcher',relatedLoadId:activeLoad.id,relatedPage:'dispatch',level:alert.severity,createdBy:'system'}); addNotification(db,{title:alert.title,text:alert.text,type:'Delay Alert',targetRole:'admin',relatedLoadId:activeLoad.id,relatedPage:'dispatch',level:alert.severity,createdBy:'system'}); }
        }
      }
      logAudit(db, 'Driver location updated', `${driverRecord.name} ${lat},${lng}`, authUser.email); writeDb(db);
      sendJson(res, 201, { ok: true, location: loc, rest }); return;
    }
    const restActionMatch = /^\/api\/rest-events\/([^/]+)\/(confirm|reject)$/.exec(pathName);
    if (req.method === 'POST' && restActionMatch) {
      const event = db.restEvents.find(item => String(item.id) === String(restActionMatch[1]));
      if (!event) return sendJson(res, 404, { error: 'Rest event not found.' });
      const canReview = authUser.role === 'admin' || (authUser.role === 'dispatcher' && event.dispatcherId === authUser.id) || (authUser.role === 'driver' && event.driverUserId === authUser.id);
      if (!canReview) return sendJson(res, 403, { error: 'You are not authorized to review this rest event.' });
      const body = await readJsonBody(req); const action = restActionMatch[2];
      if (authUser.role !== 'driver' && action === 'confirm' && !body.driverConfirmedAt) return sendJson(res, 403, { error: 'Only the Driver can confirm a browser-GPS rest event.' });
      event.state = action === 'confirm' ? 'Driver Confirmed Rest' : 'Driver Rejected Rest'; event.classification = sanitizeText(body.classification || (action === 'confirm' ? 'Resting / Off Duty' : 'Not Resting')); event.note = sanitizeText(body.note || '').slice(0,1000); event.confirmedAt = new Date().toISOString(); event.confirmedBy = authUser.email;
      if (action === 'confirm') {
        event.hosSource = db.company.eldProvider && db.company.eldProvider !== 'manual' ? 'ELD comparison' : 'Driver confirmation';
        event.hosLabel = db.company.eldProvider && db.company.eldProvider !== 'manual' ? 'Compared with ELD' : 'Estimated HOS · Driver Confirmation Required';
      }
      addNotification(db,{title:action==='confirm'?'Rest confirmed':'Possible rest rejected',text:`${event.driverName} classified the period as ${event.classification}.`,type:'Rest review',targetRole:'dispatcher',relatedLoadId:event.loadId,relatedPage:'eld',createdBy:authUser.email});
      if (authUser.role !== 'driver') addNotification(db,{title:action==='confirm'?'Rest reviewed':'Rest rejected',text:`Rest event for ${event.driverName} was reviewed by operations.`,type:'Rest review',target:event.driverEmail,targetName:event.driverName,role:'driver',relatedLoadId:event.loadId,relatedPage:'driver-mobile',createdBy:authUser.email});
      logAudit(db, action === 'confirm' ? 'Rest confirmed' : 'Rest rejected', `${event.driverName} ${event.id}`, authUser.email); writeDb(db);
      sendJson(res,200,{ok:true,event}); return;
    }
    if (req.method === 'GET' && pathName === '/api/ops/eta') {
      const loadId = sanitizeText(url.searchParams.get('loadId') || ''); const load = db.loads.find(item=>String(item.id)===loadId);
      if (!load || !loadVisibleToUser(db, authUser, load)) return sendJson(res,404,{error:'Load not found.'});
      const estimate = operationalEstimateForLoad(db, load); sendJson(res,200,estimate); return;
    }
