...
async function subscribeIfPossible({ forcePrompt=false } = {}) {
  ...
  if (!keyInfo?.enabled || !keyInfo?.publicKey) return { ok:false, reason:keyInfo?.reason || "missing_vapid" };
...
function explainReason(reason) {
  switch (reason) {
    ...
    case "missing_package":
      return "Server has VAPID keys, but the web-push package is not installed. Redeploy the latest version with npm install.";
    case "missing_keys":
    case "missing_vapid":
