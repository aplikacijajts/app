...
const LOCATION_CONSENT_PREFIX = 'jtsLocationConsent:';
const locationUiState = { consent: false, status: 'Not requested', session: null, lastError: '', restEvent: null };
function locationConsentKey() { return `${LOCATION_CONSENT_PREFIX}${state.currentUser?.id || state.currentUser?.email || 'unknown'}`; }
function hasStoredLocationConsent() { return localStorage.getItem(locationConsentKey()) === 'true'; }
function setStoredLocationConsent(value) { localStorage.setItem(locationConsentKey(), value ? 'true' : 'false'); locationUiState.consent = !!value; }
const loaderState = { active: 0, key: '', timer: null };
function ensureJtsLoaderRoot() {
  let root = document.getElementById('jtsLoaderRoot');
  if (!root) { root = document.createElement('div'); root.id = 'jtsLoaderRoot'; root.className = 'jts-loader-root'; root.setAttribute('aria-live','polite'); root.setAttribute('aria-atomic','true'); document.body.appendChild(root); }
  return root;
}
function showJtsLoader(options = {}) {
  const root = ensureJtsLoaderRoot(); loaderState.active += 1; loaderState.key = options.key || loaderState.key || 'default';
...
function updateJtsLoader(options = {}) { if (!loaderState.active) return showJtsLoader(options); return showJtsLoader({ ...options, compact: false }); }
function stopLocationWatch() { if (state.liveGpsWatchId !== null && navigator.geolocation) { try { navigator.geolocation.clearWatch(state.liveGpsWatchId); } catch (error) {} } state.liveGpsWatchId = null; state.gpsSharing = false; }
...
const mobileNav = { admin: ['dashboard', 'dispatch', 'loads', 'chat', 'notifications'], dispatcher: ['dashboard', 'dispatch', 'loads', 'chat', 'notifications'], driver: ['driver-mobile', 'documents', 'gps', 'notifications', 'fuel'], broker: ['dashboard', 'loads', 'documents', 'chat', 'notifications'] };
...
const rolePages = {
  ...
  driver: ['driver-mobile', 'documents', 'fuel', 'chat', 'gps', 'notifications'],
...
async function registerPwaServiceWorker() {
  ...
  try {
    ...
    await Promise.race([reg.update().catch(() => {}), new Promise(resolve => window.setTimeout(resolve, 5000))]);
  } catch (error) {
...
async function api(url, options = {}) {
  const token = storedToken(); const existingHeaders = options.headers || {};
  const headers = options.body instanceof FormData ? { ...existingHeaders } : { 'Content-Type': 'application/json', ...existingHeaders };
  if (token) headers.Authorization = `Bearer ${token}`;
  const controller = new AbortController(); const timeoutMs = Number(options.timeoutMs || API_DEFAULT_TIMEOUT_MS); const timer = window.setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(url, { ...options, headers, signal: options.signal || controller.signal });
    const type = response.headers.get('content-type') || ''; const payload = type.includes('application/json') ? await response.json() : await response.text();
    if (!response.ok) {
      if (response.status === 401 && !url.includes('/api/login') && !url.includes('/api/setup') && !url.includes('/api/bootstrap')) { clearSession(); showLogin(true); }
...
async function startSession(payload) {
  ...
  locationUiState.consent = hasStoredLocationConsent();
...
async function restoreSession() {
  ...
  state.authToken = token; state.currentUser = user; state.role = user.role;
  locationUiState.consent = hasStoredLocationConsent();
  showJtsLoader({ key:'restore', label:'Restoring secure session', stage:'Checking authentication and loading operational data' });
  try {
    await loadData();
    await loadSystemHealth();
    qs('#loginScreen').classList.add('is-hidden');
    ...
    clearSession();
    toast(error.message || 'Secure session could not be restored. Please sign in again.');
    return false;
  } finally {
    hideJtsLoader(true);
  }
...
function titleForPage(id) {
  ...
  if (id === 'gps' && role() === 'driver') return { ...item, label: 'Driver Location', kicker: 'Live location' };
  return item;
}
...
function accessibleGroups() {
  return navGroups.map(group => {
    let items = group.items.filter(item => canOpen(item.id) && !(role() === 'driver' && item.id === 'eld'));
    if (role() === 'driver' && group.title === 'Control') {
      const chatIndex = items.findIndex(item => item.id === 'chat');
      const gps = items.find(item => item.id === 'gps');
      // Driver Location occupies the exact menu position previously used by Chat.
      items = items.filter(item => !['chat','gps'].includes(item.id));
      if (gps && chatIndex >= 0) items.splice(chatIndex, 0, { ...gps, label: 'Driver Location', kicker: 'Live location' });
    }
    return { ...group, items };
...
async function handleAction(action, element) {
  ...
  const map = {
    ...
    'send-broker-operational-message': () => sendBrokerOperationalMessage(),
    'refresh-map': () => refresh(),
    'chat-attach-file': () => {
    ...
    'confirm-rest': () => openRestClassificationModal(element?.dataset?.rest,true),
    'reject-rest': () => openRestClassificationModal(element?.dataset?.rest,false),
...
async function sendBrokerOperationalMessage(){ if(role()!=='broker') throw new Error('Broker access required.'); const driver=qs('#brokerMessageDriver')?.value; const text=qs('#brokerOperationalMessage')?.value?.trim()||''; if(!driver||!text) throw new Error('Select an authorized Driver and enter a message.'); const btn=qs('#brokerOperationalSend'); if(btn)btn.disabled=true; try{showJtsLoader({compact:true,label:'Sending message',stage:'Routing to Admin and the assigned Dispatcher'}); await api('/api/broker-message',{method:'POST',body:JSON.stringify({driverId:driver,text})}); qs('#brokerOperationalMessage').value=''; toast('Operational message sent to Admin and Dispatcher'); await refresh();}finally{if(btn)btn.disabled=false;hideJtsLoader(true);} }
...
async function init() {
  ...
  qs('#logoutBtn').addEventListener('click', async () => {
    ...
    await handleLogoutLocationCleanup();
  ...
  window.addEventListener('beforeunload', () => {
    if (state.currentUser?.role === 'driver' && storedToken()) { fetch('/api/location/stop',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${storedToken()}`},body:'{}',keepalive:true}).catch(()=>{}); }
