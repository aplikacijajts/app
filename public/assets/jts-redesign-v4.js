(function(){
  ...
  const routes={
    admin:[['/admin.html','Console','grid'],['/admin-users.html','Team','users'],['/admin-loads.html','Loads','truck'],['/chat.html','Messages','chat'],['/notifications.html','Alerts','bell'],['/gps.html','GPS','map']],
    ...
    driver:[['/driver.html','Portal','grid'],['/load-details.html','Loads','truck'],['/chat.html','Messages','chat'],['/notifications.html','Alerts','bell']],
    broker:[['/broker.html','Workspace','grid'],['/admin-loads.html','Loads','truck'],['/chat.html','Messages','chat'],['/notifications.html','Alerts','bell'],['/gps.html','GPS','map']]
  };
  const icons={
  ...
  function user(){try{return window.JTSAuth?.currentUser?.()||null}catch{return null}}
  ...
  function nav(){
    if(isAuthPage())return; const u=user(); if(!u)return; document.querySelectorAll('.jts-rail,.jts-menu-root,.jts-mobile-menu,.pe-bottom-nav,.enterprise-dock').forEach(x=>x.remove()); const items=routes[role()]||routes.driver;
    ...
    const root=document.createElement('div'); root.className='jts-menu-root'; root.innerHTML=`<button class="jts-menu-fab" type="button" aria-label="Menu">${icons.menu}</button><div class="jts-menu-backdrop"></div><nav class="jts-menu-sheet"><div class="jts-menu-title"><strong>Menu</strong><span>${u.role||'user'}</span></div><div class="jts-menu-list"></div><button class="jts-menu-logout" type="button"><span class="jts-menu-icon">${icons.out}</span><span>Sign out</span></button></nav>`;
  ...
  function loginEnhance(){
    if(!isAuthPage())return; const box=document.querySelector('main>div.bg-white, main>div.border'); if(!box)return;
    box.querySelectorAll('.jts-auth-logo').forEach((el,i)=>{ if(i>0) el.remove(); });
    if(box.querySelector('.jts-auth-logo')) return;
    const logo=document.createElement('div'); logo.className='jts-auth-logo'; logo.innerHTML='<img src="/assets/jts-logo.png" alt="JTS" style="height:46px;margin-bottom:18px">'; box.insertBefore(logo,box.firstChild);
  }
